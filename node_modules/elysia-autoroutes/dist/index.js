...
function transformPathToUrl(filePath) {
    ...
    ;
    const resultUrl = url.split(path__default.default.sep).map((part)=>handleParameters(part)).join('/') // Map and join the URL parts using handleParameters function
    ;
...
async function autoload(app, routesDir, generateTags) {
    ...
    if (!fs__default.default.existsSync(dirPath)) throw new Error(`Directory "${dirPath}" does not exist`);
    if (!fs__default.default.statSync(dirPath).isDirectory()) throw new Error(`"${dirPath}" is not a directory.`);
...
function getDirPath(dir) {
    let dirPath;
    if (path__default.default.isAbsolute(dir)) dirPath = dir;
    else if (path__default.default.isAbsolute(process.argv[1])) dirPath = path__default.default.join(process.argv[1], '..', dir);
    else dirPath = path__default.default.join(process.cwd(), process.argv[1], '..', dir);
    return dirPath;
}
...
function autoroutes(options) {
    ...
    const autoroutesPlugin = new elysia.Elysia({
        prefix,
